...
File: reliance_digital_scraper.py

Description:
Scrapes Reliance Digital product listings and stores the
...
def scrape_product(page, listing):
    """
    Scrapes a single Reliance Digital product.
    """
    ...
    page.wait_for_selector("h1.product-name")
    ...
    button_locator = page.locator("button.add-card-btn")
    price_locator = page.locator("div.product-price")

    if button_locator.count() > 0 and price_locator.count() > 0:
...
def main():
    ...
    listings = fetch_product_listings("Reliance Digital")
